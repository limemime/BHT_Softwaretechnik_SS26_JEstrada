...
# We import selenium to allow our engine to open a
# headless browser in the background for automated searching.
...
class EntropyEngine:
    ...
    def __init__(self, filepath, min_sleep, max_sleep):
        ...
        # We keep track of the total number of searches performed.
        self.search_count = 0
        
        # We declare a variable to hold the selenium webdriver instance.
        self.driver = None
    ...
    def _run_loop(self):
        # We configure the Selenium WebDriver to run in headless mode
        # so it doesn't interrupt the user interface.
        chrome_options = Options()
        chrome_options.add_argument("--headless")
        # We initialize the WebDriver using the configured options.
        self.driver = webdriver.Chrome(options=chrome_options)
        
        # We log that the driver successfully initialized.
        logging.info("Headless Chrome WebDriver initialized.")
        ...
        while self.running:
            ...
            # We use Selenium to navigate to the search URL quietly.
            try:
                self.driver.get(url)
                # We increment the search counter since a search was attempted.
                self.search_count += 1
            except Exception as e:
                logging.error(f"Error during search navigation: {e}")
        ...
        # When the loop breaks, we properly close the driver.
        if self.driver:
            self.driver.quit()
            self.driver = None

...
class DashboardApp:
    ...
    def __init__(self, root):
        ...
        # We create a string variable to track the total
        # number of background searches completed so far.
        self.metrics_var = tk.StringVar(value="Total Searches: 0")
        
        # We create a themed label to display the metrics
        # clearly to the user.
        self.metrics_label = ttk.Label(self.frame, textvariable=self.metrics_var, font=("Arial", 11, "bold"))
        
        # We place the metrics label at the very bottom
        # of the grid.
        self.metrics_label.grid(row=5, column=0, columnspan=2, pady=(15, 0))
        
        # We start the background polling process to continuously
        # fetch the latest search count from the engine.
        self.update_metrics()
    ...
    # This method continuously polls the engine for the latest
    # search count and updates the graphical user interface.
    def update_metrics(self):
        # We fetch the current search count directly from the engine instance.
        count = self.engine.search_count
        # We dynamically update the string variable tied to the metrics label.
        self.metrics_var.set(f"Total Searches: {count}")
        # We use the root.after method to schedule this function
        # to run again in 1000 milliseconds (1 second).
